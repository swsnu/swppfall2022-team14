...
class CocktailViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        # Add params Filter
        try:
            self.process_get_list_params(request, filter_q)
        except (ValueError, AssertionError) as e:
            return HttpResponseBadRequest('Invalid ABV or Filter Type', e)

        # Add ingredient filter
        self.get_cocktail_list_by_ingredient(request, filter_q)

        # Add text filter
        self.process_text_param(request, filter_q)
        ...
        cocktails = self.queryset.filter(filter_q)
        data = self.serializer_class(cocktails, many=True).data
    ...
    def retrieve(self, request, pk):
        try:
            cocktail = self.queryset.get(id=pk)
    ...
    def retrieve_my_cocktail(self, request):
        if request.method == 'GET':
            # TODO: author_id=request.user.id
            cocktails = self.queryset.filter(author_id=1, type='CS')
            data = self.serializer_class(cocktails, many=True).data
            return JsonResponse({"cocktails": data, "count": cocktails.count()}, safe=False)
    ...
    def process_text_param(self, request, filter_q):
    ...
    def process_get_list_params(self, request, filter_q):
    ...
    def get_cocktail_list_by_ingredient(self, request, filter_q):
        ...
        cocktail_all = self.queryset.all()
