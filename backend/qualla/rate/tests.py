...
class RateTestCase(TestCase):
    ...
    def test_rate_list_user_modify(self):
        ...
        response = self.client.post(
            '/api/v1/rates/3/user/', json.dumps({"score": 0}), content_type='application/json')
        self.assertEqual(response.status_code, 201)

        response = self.client.post(
            '/api/v1/rates/10/user/', json.dumps({"score": 0}), content_type='application/json')
        self.assertEqual(response.status_code, 404)
        ...
        response = self.client.put(
            '/api/v1/rates/1/user/', json.dumps({"score": 1}), content_type='application/json')
        self.assertEqual(response.status_code, 200)

        response = self.client_norate.put(
            '/api/v1/rates/1/user/', json.dumps({"score": 1}), content_type='application/json')
        self.assertEqual(response.status_code, 404)

        response = self.client.put(
            '/api/v1/rates/10/user/', json.dumps({"score": 1}), content_type='application/json')
        self.assertEqual(response.status_code, 404)
